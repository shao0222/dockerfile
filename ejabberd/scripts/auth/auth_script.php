...
abstract class EjabberdExternalAuth {
    ...
    final private function work() {
        ...
        while (true) {
            ...
            //$this->log('Received message: ' . json_encode($message), LOG_INFO);
...
class MyAuth extends EjabberdExternalAuth {
    ...
    protected function authenticate($user, $server, $password) {
        $this->log('authenticate: ' . $user . '@' . $server . '/' . $password, LOG_INFO);
        return true;
    }

    protected function exists($user, $server) {
        $this->log('exists: ' . $user . '@' . $server, LOG_INFO);
        return true;
    }

}

new MyAuth('/opt/ejabberd/logs/auth_script.log');
